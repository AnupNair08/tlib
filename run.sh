...
for i in "${tests[@]}"
do
   echo -n "Input size $i x $i => " && { time python3 src/OneOne/tests/script.py $i | ${OneOneTest}/benchmark single > /dev/null; } |& grep real  
done
# echo "======================================"
printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' =
...
for i in "${tests[@]}"
do
   echo -n "Input size $i x $i => " && { time python3 src/OneOne/tests/script.py $i | ${OneOneTest}/benchmark multi > /dev/null; } |& grep real  
done
printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' =
...
do
   sec=` expr $i / 1000000 `
   ms=` expr $i % 1000000 `
   echo -n "Scheduling Interval $sec s $ms μs => " && { time python3 src/OneOne/tests/script.py 500 | ${ManyOneTest}/matrix multi $i > /dev/null; } |& grep real  
done
printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' =
...
for i in "${tests[@]}"
do
   echo -n "Input size $i x $i => " && { time python3 src/OneOne/tests/script.py $i | ${ManyOneTest}/matrix single > /dev/null; } |& grep real  
done
# echo "======================================"
printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' =
